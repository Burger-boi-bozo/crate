/* Crate v4 controls, detailed progress, diagnostics, and job activity. */
...
async function showActivity(job, row, button) {
  let panel = row.querySelector('.job-activity');
  if (panel) { panel.remove(); button.textContent = 'Activity'; return; }
  button.disabled = true;
  try {
    const events = await api(`/api/jobs/${encodeURIComponent(job.id)}/events`);
    panel = element('div', 'job-activity');
...
renderJobs = function enhancedRenderJobs() {
  ...
  [...document.querySelectorAll('#job-list .job')].forEach((row, index) => {
    ...
    if (!actions) return;
    if (!actions.querySelector('[data-crate-primary]')) {
      let control;
      ...
      if (control) { control.dataset.cratePrimary = '1'; actions.prepend(control); }
    }
    if (jobs.some(current => current.id === job.id) && !actions.querySelector('[data-crate-activity]')) {
      const activity = element('button', 'text-button', 'Activity');
      activity.dataset.crateActivity = '1';
      activity.onclick = () => showActivity(job, row, activity);
