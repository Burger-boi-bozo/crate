...
def install(app, queue, config, owner):
    ...
    async def delete(job_id: str, request):
        job = queue.owned(job_id, owner(request))
    ...
    async def download(job_id: str, request):
        queue.expire()
        job = queue.owned(job_id, owner(request))
