"""Persistent concurrent queue, controls, event history, and live updates."""
...
class Queue:
    ...
    async def start(self):
        ...
        for job in self.jobs.values():
            if job.get("status") in ACTIVE:
                ...
                self.record(job, "recovered", "Recovered after a server restart")
        self.expire()
        self.save()
    ...
    def notify(self, job: dict, kind: str, event: dict | None = None):
    ...
    def record(self, job: dict, kind: str, message: str, payload: dict | None = None):
        event = self.store.add_event(job, kind, message, payload)
        self.notify(job, "job", event)
        return event

    def progress_changed(self, job: dict):
    ...
    def submit(self, owner: str, body: Submission):
        ...
        if reserve and usage.free < reserve:
            raise HTTPException(503, "Storage reserve is low. Finish or remove older downloads before starting another job.")
        job_id = secrets.token_hex(16)
        job = dict(id=job_id, owner=owner, url=body.url, provider=urlsplit(body.url).hostname or "unknown",
                   format=body.format, quality=body.quality, title=body.url,
                   status="queued", stage="queued", progress=0, downloaded_bytes=0, total_bytes=0,
                   speed=None, eta=None, conversion_progress=None, created_at=now, finished_at=None,
                   expires_at=None, error=None, error_code=None, diagnostic=None, size=None,
                   width=None, height=None, filename=None, path=None, serves=0, paused_from=None)
        self.jobs[job_id] = job
        self.daily.append(now)
        self.pending.put_nowait(job_id)
        self.save()
        self.record(job, "queued", "Added to the queue")
        return job
    ...
    async def cancel(self, job):
        ...
        job["path"] = None
        self.save()
        self.record(job, "cancelled", "Cancelled")
        return job
    ...
    async def resume(self, job):
        ...
        if process and process.returncode is None:
            ...
            self.pending.put_nowait(job["id"])
        self.save()
        self.record(job, "resumed", "Resumed")
        return job

    def retry(self, owner: str, job):
        if job.get("status") in ACTIVE:
            raise HTTPException(409, "Cancel or finish this job before retrying it.")
        new_job = self.submit(owner, Submission(url=job["url"], format=job["format"], quality=job.get("quality", "best")))
        self.record(new_job, "retry", "Retried a previous job", {"source_job_id": job["id"]})
        return new_job
    ...
    def expire(self):
        ...
        for job_id, job in list(self.jobs.items()):
            if job.get("expires_at") and job["expires_at"] <= now and job.get("status") == "ready":
                ...
                self.record(job, "expired", "File expired")
